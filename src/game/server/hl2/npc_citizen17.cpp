		...
		if ( pPlayerAISquad )
		{
			...
			for ( CAI_BaseNPC *pAllyNpc = pPlayerAISquad->GetFirstMember(&iter); pAllyNpc; pAllyNpc = pPlayerAISquad->GetNextMember(&iter) )
			{
				if ( pAllyNpc->GetCommandGoal() != vec3_invalid )
				{
					...
					pAllyNpc->TargetOrder( UTIL_GetNearestVisiblePlayer(this), &pAllyNpc, 1 );
					if ( !bHadGag )
...
void CNPC_Citizen::PredictPlayerPush()
{
	...
	if ( pPlayer && !bHadPlayerPush && HasCondition( COND_PLAYER_PUSHING ) && 
...
bool CNPC_Citizen::ShouldAutoSummon()
{
	...
	if ( !pPlayer || !IsFollowingCommandPoint() || !IsInPlayerSquad() )
		return false;
...
void CNPC_Citizen::UpdateFollowCommandPoint()
{
	if ( IsInPlayerSquad() )
	{
		...
		else
		{
			...
			if (pNearest && m_FollowBehavior.GetFollowTarget() != pNearest)
			{
...
{
	TossHealthKit( UTIL_GetNearestVisiblePlayer(this), Vector(48.0f, 0.0f, 0.0f)  );
}
